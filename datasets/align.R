            ...
            withProgress({
                ...
                for (f in 1:nrow(fileinfo)) {
                    ...
                    if (tolower(ext) == "csv") {
                        ...
                        this.raw = as.data.table(read_csv(fileinfo[f,]$datapath, col_names = header, na = default$missing_values, skip = input$data_unaligned_header_row - 1, show_col_types = F))
                    ...
                    for (i in 1:len.x) {
                        while (j <= len.y) {
                            if (j != len.y & (abs(y[j+1] - x[i]) < abs(y[j] - x[i]))) {
                                j = j + 1
                            } else {
                                break
                            }
